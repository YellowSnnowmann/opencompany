...
| `tools.rs` / `tools_tests.rs` | `InFlightRegistry` — one in-flight turn per agent, keyed by runtime agent id — the `InFlight` speech fold (`speech::interpret`, one action per turn, `dm` recipient check), `InFlightContext` (the `ToolRunContext` a served tool runs under) and `McpToolAdapter`. Gated on `mcp`. |
