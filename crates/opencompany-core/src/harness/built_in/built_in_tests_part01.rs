...
async fn roster_builds_every_manifest_agent() {
    let fx = fixture();
    let roster = build_roster(&test_runtime(), &crate::harness::openhuman_runtime::global_blocking(crate::harness::openhuman_runtime::RuntimeBoot::ephemeral()).expect("runtime"), &record(), &fx.deps, &[], &HashMap::new()).expect("roster builds");
