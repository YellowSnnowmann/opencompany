...
async fn roster_builds_with_skill_surface_wired() {
    ...
    let roster =
        build_roster(&test_runtime(), &crate::harness::openhuman_runtime::global_blocking(crate::harness::openhuman_runtime::RuntimeBoot::ephemeral()).expect("runtime"), &record(), &deps, &[], &HashMap::new()).expect("roster builds with skills");
...
async fn overlay_agent_is_built_as_a_real_roster_agent() {
    ...
    let roster = build_roster(&test_runtime(), &crate::harness::openhuman_runtime::global_blocking(crate::harness::openhuman_runtime::RuntimeBoot::ephemeral()).expect("runtime"), &rec, &fx.deps, &[], &HashMap::new()).expect("roster builds");
...
async fn a_tool_added_teammate_colliding_with_a_manifest_id_still_joins_the_roster() {
    ...
    let roster = build_roster(&test_runtime(), &crate::harness::openhuman_runtime::global_blocking(crate::harness::openhuman_runtime::RuntimeBoot::ephemeral()).expect("runtime"), &saved, &fx.deps, &[], &HashMap::new()).expect("roster builds");
    let ids: Vec<_> = roster.iter().map(|a| a.agent_id.as_str()).collect();
    assert_eq!(
        ids,
