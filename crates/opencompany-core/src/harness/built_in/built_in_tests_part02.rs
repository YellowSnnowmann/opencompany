...
async fn roster_builds_with_skill_surface_wired() {
    ...
    let roster =
        build_roster(&test_runtime(), &record(), &deps, &[], &HashMap::new()).expect("roster builds with skills");
...
async fn a_tool_added_teammate_colliding_with_a_manifest_id_still_joins_the_roster() {
    ...
    let roster = build_roster(&test_runtime(), &saved, &fx.deps, &[], &HashMap::new()).expect("roster builds");
    let ids: Vec<_> = roster.iter().map(|a| a.agent_id.as_str()).collect();
    assert_eq!(
        ids,
