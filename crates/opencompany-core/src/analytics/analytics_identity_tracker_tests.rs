//! Tenant-id opacity, coarse failure classes, and the tracker lifecycle: split
//! out of `analytics_tests.rs` (topic split, >750 lines) so it stays a
//! sibling `use super::*;` module of `analytics`. `envelope()` and
//! `hostile_events()` are duplicated from `analytics_payload_tests.rs`
//! because both files need them and each is its own module.

use super::*;
use crate::analytics::types::{OpaqueId, TenantIdKey, provider_slug};
