...
fn main() {
    ...
    for entry in entries.filter_map(Result::ok) {
        ...
        // `companies/_globals` is the global baseline, embedded by
        // `embed_globals` below; it is not a bundle anyone can serve.
        if id.starts_with('_') {
            continue;
        }
...
fn embed_globals(root: &Path) {
    ...
    // The baseline's seed board cards. A file at the baseline's root, so the
    ...
    out.push_str(&format!(
        "/// `companies/_globals/globals.toml`, verbatim (empty when the directory is absent).\n\
         pub static EMBEDDED_GLOBALS_MANIFEST: &str = {manifest:?};\n"
    ));
    out.push_str(&format!(
        "/// `companies/_globals/tasks.toml`, verbatim (empty when the file is absent).\n\
         pub static EMBEDDED_GLOBAL_TASKS: &str = {tasks:?};\n"
    ));
    write_pairs(
        &mut out,
        "EMBEDDED_GLOBAL_AGENTS",
        "Every file under `companies/_globals/agents/`, keyed by path relative to it, sorted.",
        &agents,
    );
    write_pairs(
        &mut out,
        "EMBEDDED_GLOBAL_WORKFLOWS",
        "Every file under `companies/_globals/workflows/`, keyed by path relative to it, sorted.",
        &workflows,
    );
    write_pairs(
        &mut out,
        "EMBEDDED_GLOBAL_LEDGERS",
        "Every file under `companies/_globals/ledgers/`, keyed by path relative to it, sorted.",
        &ledgers,
    );
    write_pairs(
        &mut out,
