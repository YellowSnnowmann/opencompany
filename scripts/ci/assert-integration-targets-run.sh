...
# The host crate declares every integration target explicitly (`autotests =
# false` in `crates/opencompany-core/Cargo.toml`), so a file dropped into its
# `tests/` without a `[[test]]` entry is built by nothing and run by nothing —
...
for file in crates/opencompany-core/tests/*.rs; do
  [ -e "${file}" ] || continue
  stem="$(basename "${file}" .rs)"
  if ! grep -qx "${stem}" "${WORK}/targets"; then
    echo "::error title=Integration test not declared::${file} has no [[test]] entry in crates/opencompany-core/Cargo.toml, so no lane builds or runs it. Add \`[[test]] name = \"${stem}\" path = \"tests/${stem}.rs\"\`." >&2
    exit 1
  fi
done
