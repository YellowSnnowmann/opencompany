...
/**
 * A model id belongs to a provider it is chosen from. Keys rework (issue
 * #2306, decision D-model): every model step this field renders now REQUIRES
 * a non-blank id — there is no more "leave it blank and send the tier"
 * passthrough, because no tier name is ever sent as a model any more (2d).
 * `TIER_DEFAULT_LABEL` and the blank catalogue row it used to render are gone;
 * do not resurrect them.
 */
export const CHOOSE_MODEL_LABEL = "Choose a model";
 ...
 *   from a stale catalog (including a previously-chosen model the provider no
 *   longer lists). Upgrading the field out from under it would discard it,
 *   which is the same class of bug as stripping a value mid-keystroke.
 */
...
/**
 * Choosing a model id for one provider — always required (keys rework, issue
 * #2306). **A catalog select with a free-text escape hatch**, not one or the
 * other. The catalog is what makes this usable — an operator who has to know a
 * vendor's id scheme by heart is being asked the wrong question — and the
 * escape hatch is what keeps it correct.
 *
 * ## Two ways this field gets its list
 *
 * **Fetch mode** (`models` omitted): the field reads `slug`'s own catalogue
 * live, via `GET …/providers/{slug}/models` — used to change an *existing*
 * row's model or the company default, where the provider is already saved and
 * its key already lives on the host.
 *
 * **List mode** (`models` given): nothing is fetched — the caller already has
 * the list, typically from a draft probe run before anything was saved (the
 * add flow: type a key, the model step opens with that same probe's answer).
 * `slug` may be `null` in this mode, because there is no saved row yet to ask.
 *
 ...
 * never contain. `freeTextOnly` defaults to text there; everywhere else the
 * catalog is the default and the toggle is one click away, because a catalog
 * can be stale or incomplete anywhere.
 *
 ...
 *
 * The catalog can arrive asynchronously (fetch mode) and the field starts as
 * text. When the list lands it becomes a select **only if nothing has been
 * typed** — upgrading a field out from under someone mid-keystroke is the same
 * class of bug as stripping a value mid-keystroke, which this module's
 * neighbour has nine recorded instances of.
 */
export function ModelField({
  ...
  company: string | null;
  /** The provider whose catalog to read live, or `null` when `models` is given or none is chosen yet. */
  slug: string | null;
  id: string;
  label?: string;
  value: string;
  ...
  /** List mode: a catalogue already in hand (e.g. from a draft probe). Nothing is fetched when given. */
  models?: readonly string[];
  /** List mode only: forces the free-text field (an Azure endpoint's own answer). */
  freeTextOnly?: boolean;
  /** List mode only: why the list is empty, when that is worth saying. */
  catalogError?: string;
}) {
  const listMode = models !== undefined;
  const [fetched, setFetched] = useState<ProviderCatalog | null>(null);
  ...
  useEffect(() => {
    ...
    listProviderModels(client, company, slug)
      .then((next) => live && setFetched(next))
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- `listMode` is a prop, not state; re-running on it would refetch for no reason
  }, [client, company, slug]);

  const catalog: ProviderCatalog | null = listMode
    ? { baseUrl: "", models: [...(models ?? [])], freeTextOnly: Boolean(freeTextOnly), error: catalogError }
    : fetched;
  const listed = catalog?.models ?? [];
  const offersSelect = showsCatalogSelect({ catalog, typed, value });
  // In list mode the caller already has a catalogue in hand (or knows it does
  // not), so there is nothing to be "chosen" against beyond that — a `null`
  // `slug` there is not "no provider yet", it is "not asking per-provider".
  const chosen = listMode || Boolean(slug);
    ...
    <div className="grid gap-1.5">
      ...
      {offersSelect ? (
        ...
        <Input
          ...
          placeholder={chosen ? "Type a model id" : "Choose a provider first"}
          autoComplete="off"
          spellCheck={false}
...
function ModelFieldNote({
  ...
  loading: boolean;
  /** Whether a provider has been chosen at all (or list mode is feeding the field directly). */
  chosen: boolean;
 ...
 * it is a haystack with a scrollbar. TinyHumans' own proxy catalogue is capped
 * at 500 the same way. The ranking is [`filterModels`](./model-filter.ts),
 * which is `search/rank.ts` underneath.
 *
 * ## Every row here is a model — a real id is always required
 *
 * Unlike the field this replaced, there is no blank "send the tier" row: a
 * model is required everywhere this component is used (D-model), and no tier
 * name is ever sent as one (2d). An unset value shows {@link CHOOSE_MODEL_LABEL}
 * on the trigger and nothing is pre-selected.
 *
 ...
 * endpoint or against a stale catalog, so one control does both jobs rather
 * than dead-ending into a separate toggle.
 *
    ...
    >
      ...
      <PopoverContent align="start" className="w-(--anchor-width) p-0">
        ...
        {rows.length > 0 ? (
          ...
          <p className="p-2 text-xs text-muted-foreground" data-testid="inference-model-empty-list">
