...
The demo does not ship a default username or password. Create a demo user with
administrator access once for each fresh data volume. The helper securely
prompts for a password, boots and stops the backend as needed, then writes the
credential:
...
Open <http://localhost:5173> and sign in as `you@example.com` with the password
you entered. Use the same demo name in both commands: `marketing` above can be
replaced by any name printed by `./scripts/list-demos.sh`.
