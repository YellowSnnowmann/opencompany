...
| --- | --- | --- |
| `managed` | `search/managed/key`, then the deployment fallback | — |
...
3. A credential exists: either a company provider that is *complete*, or the
   managed backend through the company's TinyHumans key or deployment fallback.
   Neither, and nothing is wired — fail-closed, with a warning naming which half
   is missing.
...
| Key | Read back? | Holds |
| --- | --- | --- |
| `search/managed/key` | **never** | the company's copied TinyHumans key |
| `search/provider` | yes | the selected slug |
| `search/api_key` | **never** | deprecated entry-zero BYO fallback |
...
There is deliberately **no environment fallback for a BYO provider key**: an
ambient one could only ever be somebody else's account. Managed search is a
separate surface and does fall back from the company's TinyHumans key to the
deployment identity. A selected-but-incomplete provider is not an error — it
falls back to managed, which is what OpenHuman's own registry does for a BYO
engine with no key. The status route reports
